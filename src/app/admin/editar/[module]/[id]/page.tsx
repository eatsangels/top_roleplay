...
export default async function EditAdminRecordPage({
  ...
  const { data: record, error } = await supabase.from(form.table).select("*").eq("id", id).maybeSingle();
  if (error || !record) notFound();
  const fieldOptions = await loadFieldOptions(supabase, form.fields, record);

  return (
    <AdminLayout userEmail={user.email} userName={user.user_metadata?.username} userRole={role}>
      <Card className="mx-auto max-w-3xl p-6">
        <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-magic">Registro existente</p>
        <h2 className="mt-2 font-fantasy text-3xl font-black text-white">{form.editTitle}</h2>
        ...
        <form action="/api/admin/update-record" className="mt-7 grid gap-5" method="post">
          <input name="id" type="hidden" value={id} />
          <input name="module" type="hidden" value={module} />
          {form.fields.map((field) => <FormField field={field} key={field.name} options={fieldOptions[field.name]} value={formValue(field, record[field.name])} />)}
          <button className="rounded-xl border border-gold-300/50 bg-gold-300/10 px-5 py-3 font-bold text-gold-300 hover:bg-gold-300/20" type="submit">Guardar cambios</button>
        </form>
      </Card>
    </AdminLayout>
  );
...
async function loadFieldOptions(supabase: Awaited<ReturnType<typeof requireAdmin>>["supabase"], fields: AdminFormField[], record: Record<string, unknown>) {
  ...
  await Promise.all(fields.map(async (field) => {
    ...
    if (currentValue && result[field.name] && !result[field.name].some((item) => item.value === currentValue)) {
      result[field.name].unshift({ value: currentValue, label: `Valor actual (${currentValue})` });
    }
  }));
  return result;
}

function formValue(field: AdminFormField, value: unknown) {
...
function FormField({ field, options, value }: { field: AdminFormField; options?: AdminFormOption[]; value: string }) {
  ...
  return (
    <label className="grid gap-2 text-sm font-bold text-neutral-300">
      {field.label}
      {field.type === "textarea" ? (
        <textarea className={`${classes} min-h-28`} defaultValue={value} name={field.name} placeholder={field.placeholder} required={field.required} />
      ) : field.type === "select" ? (
        <select className={classes} defaultValue={value} name={field.name} required={field.required}>
        ...
        <input className={classes} defaultValue={value} max={field.max} min={field.min} name={field.name} placeholder={field.placeholder} required={field.required} type={field.type ?? "text"} />
      )}
